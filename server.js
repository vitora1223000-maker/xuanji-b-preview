...
## 一、照见底色 👁（标题格式严格=两个井号+空格+"一、照见底色"+emoji，井号是 markdown 标题符号不是正文文字，emoji 放标题文字最后、绝不放井号前面）
...
## 二、你的天赋 ✨（标题=## 二、你的天赋 ✨·emoji放最后·机械咬死·别哭诉）
...
## 三、你的重生人格 ⚔️（标题=## 三、你的重生人格 ⚔️·emoji放最后·只讲两副人格·别扯 buff）
...
## 四、点亮你的人格 BUFF ✦（标题=## 四、点亮你的人格 BUFF ✦·emoji放最后·这是全篇颅内高潮·带"照抄"的句子原样抄·只换功能名和盘料）
...
照抄（命门·结构一字不改）："它们用的是两套完全不搭界的语言，从两个完全相反的入口进去——**我们却看到了一模一样的答案：${buffLines}。两条路，同一个终点。**"
...
## 五、别忘了你是谁 🌙（标题=## 五、别忘了你是谁 🌙·emoji放最后·拔到哲学高度·🔴这段只升华、绝不再堆星盘坐标）
🔴这段是全篇情绪最高点，任务只有一个：把 TA 托起来、给一束光。绝不许在这里重新念星盘坐标（"月亮水瓶…太阳狮子…冥王…"那种），前面论证过了，这里只拔高度。
...
末句【全篇最重一锤·必须加粗·自己编·绝不和别人一样·从这张盘内核长出来】：一句让 TA "含着泪却有了力气"的话收口，要顶到格局/哲学高度，说完就停。绝不"加油哦""走进那道光"这种软收、白开水。
...
- 🔴🔴【标题格式·绝不许井号裸奔】每段标题必须是 markdown 二级标题：两个井号 + 空格 + 标题文字 + 一个 emoji（emoji 放在标题文字【最后】）。例：「## 一、照见底色 👁」。绝不许把 emoji 放在井号前面（如「👁 ## 一」），那会让井号变成正文里裸露的 ## 符号，丑死。井号是 markdown 标题符号，不是给用户看的文字。
