...
③ 🔴🔴【本段命门·"为什么偏偏是 ${base}、不是隔壁 ${nb}"必须当场讲死·这才是"照见底色"的真正任务】光靠 ${dom} 钉不死型号——因为隔壁那个 ${nb} 也是 ${dom} 主导，**光凭 ${dom}，证明不了你是 ${base} 还是 ${nb}**（这句加粗）。真正把 TA 钉死成 ${base}、而不是冷的 ${nb} 的，是 TA 的辅助功能 ${auxTxt}：${nb} 是 ${dom}＋${nbAuxTxt}（看完抽离去算计），而 ${base} 是 ${dom}＋${aux}——TA 看透了还**忍不住为人疼、共情到自己难受**。从盘里挑一个能佐证 ${aux} 的坐标（情感/关系/水相能量那类）扣上去，一刀剖出来。🔴绝不许在这里讲 ${nb} 的好/把 TA 写成 ${nb}，${nb} 只是反例、衬出 ${aux}，最终落点永远是 ${base}。
   · 这一层那句最狠的，单独用 > 引用另起一行做成竖线重点句：**${dom} 给穿透力，${aux} 决定你把这穿透力用在懂人暖人上——这才是 ${base}，不是 ${nb}。** 🔴竖线只给这一句，别把整段塞进竖线。
   🔴加粗只给"结论句/功能名 ${dom}·${aux}/型号 ${base}"，星座坐标论证（"太阳狮子1°拱冥王"那种）不加粗——别把加粗浪费在论证细节上，那会把用户的眼睛引到次要的地方。
