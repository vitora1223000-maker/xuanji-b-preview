...
· 底色的天赋功能（正向作证·必须露代号+翻译）：${domTxt}
· 底色的劣势功能（反向作证·第二证人·必须露代号+翻译）：${infTxt} —— 它和上面那股天赋在结构上 180° 死死对立
· 重生人格（两个·不分主次）：${types}
...
② 【必须至少两到三个坐标层层咬死·禁单坐标·禁瞎编星座统计排行】从盘料里挑【两到三个最硬的坐标】，一层层把型号钉死——尤其抓那个【定生死的轴】：哪个坐标把这个人钉死在 ${base} 而不是最接近的隔壁型。一刀见血别绕弯。
...
① 砸天赋=${domTxt}（露代号+紧跟人话），是这个人最不费力最猛的本事，一两句讲完，这句加粗。
② 最弱的一面=${infTxt}——就一句陈述"这是你最费力、最够不到的那一面"（🔴注意：劣势功能=最弱最够不到，绝不能写成"最不费力"——"最不费力"是天赋，写反了就是硬伤）。立刻转下一步。🔴绝不默认 TA 为此自卑/不配得/痛苦（不是每个人都自卑），盘里没痛点坐标支撑就只客观陈述、不替 TA 安情绪。
③ 把这层逻辑写成正文（"焊死杠杆"四个字绝不许出现在正文）：${dom} 和 ${inf} 在功能结构里 180° 死对头，一头压越低另一头翘越高——**所以你"${inf}那方面"越弱，恰恰铁证了你全部的劲都压在 ${dom} 上。** 这句加粗。
④ 把这个双重作证写成正文（"最强算一次/最弱反推一次"这种公式话别原样甩给用户·要揉进上面的盘料里讲，让 TA 感到"从两个相反方向都指向同一个天赋"那一震）。
⑤ 这段【收尾重锤·加粗·单独成段】照抄："**你以为的缺点，从来不是缺点。天赋，是从缺点里生出来的。**" 这是本段的钩子，要重、要亮，别弱化在末尾。

## ⚔️ 三、你的重生人格（标题写"## ⚔️ 三、你的重生人格"·## 打头·只讲两副人格·别扯 buff）
① 砸结论：为活成自己，你锻出了两副人格——**${types}**，并列、不分主次。各一句讲它俩是什么劲（露型号、加粗、别打错别多写）。
② 两处铁证、各一句（"凭什么是TA"五个字绝不许出现在正文，直接讲）：一处=这两副人格的看家功能本就埋在 ${base} 的功能栈里（露功能代号+翻译）、平时被盖住；一处=盘里某类元素/星座能量重，正是它俩底料。点明"不是凭空给的，是被盘里某个痛点×渴望逼出来的"。
