...
· 🔴底色的辅助功能（定生死作证·把 ${base} 和隔壁 ${nb} 分开的那把刀·必须露代号+翻译）：${auxTxt} —— 真正把 TA 钉死成 ${base}、而不是冷的 ${nb} 的，是这个 ${aux}。${base}=${dom}打底＋${aux}定生死；隔壁 ${nb}=同样 ${dom}打底＋却是 ${nbAuxTxt}。${dom} 给穿透力，${aux} 决定 TA 把这穿透力用在哪——这是分水岭。
...
- **加粗·硬底线**：每个 ## 段落里【至少 2～3 处】加粗（戳心结论、金句、功能名、型号字母）。一整段一个加粗都没有=失败。加粗只给"戳人的话/功能名/型号"，星座坐标凭据（"金星处女14°"那种）不加粗。
- **竖线重点句**：每段那句最狠的金句，单独用 > 引用另起一行做成竖线重点句，让它跳出来（就像 buff 段那句）。每篇至少 2～3 处这样的竖线重点句，分散在各段。
...
② 【必须至少两到三个坐标层层咬死·禁单坐标·禁瞎编星座统计排行】从盘料里挑【两到三个最硬的坐标】，一层层把型号钉死。一刀见血别绕弯。
   🔴【这一段必须带上 MBTI 功能名·上次你一个功能都没讲=废】：${base} 之所以是 ${base}，核心是它的主导功能 ${domTxt}（露代号+紧跟人话）。光用星座坐标定型、却不点一个 MBTI 功能，用户根本不知道这跟人格类型有啥关系。必须把"星座坐标"和"功能 ${dom}"扣在一起讲（如：某星某座→所以你的 ${dom} 这股劲特别强→这正是 ${base} 的核心）。
   🔴加粗只给"结论句/功能名"，星座坐标论证（"太阳狮子1°拱冥王"那种）不加粗——别把加粗浪费在论证细节上，那会把用户的眼睛引到次要的地方。
③ 照抄下面这段、型号换成 ${base}（每篇都用·钩子）："**而这个底色，是你出生那一刻星象就写定的，后天怎么努力、怎么伪装都改不了。** 你做过的每一次性格测试，测的是你'想成为'的样子；星盘照见的，是你卸下面具之后，生来的样子。你此刻看到 ${base} 时心里'咯噔'的那一下，就是那种说不清的感觉第一次被准确说出来。"

## ✨ 二、你的天赋（标题写"## ✨ 二、你的天赋"·## 打头·机械咬死·别哭诉）
🔴🔴【本段是三条独立的路指向同一个 ${base}·三步缺一不可·顺序焊死·别顾此失彼】上一版你只抓 ${dom}、结果论证滑到隔壁 ${nb} 去了——病根=${dom} 是 ${base} 和 ${nb} 共有的，光抓 ${dom} 区分不了。这一版必须 ${dom} 打底、再用 ${aux} 把 ${nb} 正面堵死。三步：
① 【第一证人·主导打底】砸天赋=${domTxt}（露代号+紧跟人话），是这个人最猛、最不费力的本事，揉进盘里一两个坐标讲，这句结论加粗。
② 【第二证人·辅助定生死·🔴这是新加的命门·绝不能省】先点破：光有 ${dom}，TA 可能是 ${base}，也可能是隔壁那个【同样 ${dom} 主导】的 ${nb}——**${dom} 只证明 TA"看得透"，证明不了 TA 是谁。** 真正把 TA 和冷的 ${nb} 分开的，是 TA 的辅助功能 ${auxTxt}：${nb} 是 ${dom}＋${nbAuxTxt}（抽离去算计），而 ${base} 是 ${dom}＋${aux}——TA 看透了还**忍不住为人疼、共情到自己难受**。从盘里挑一个能佐证 ${aux} 的坐标（情感/关系/水相能量那类）扣上去。**${dom} 给穿透力，${aux} 决定 TA 把这穿透力用在懂人暖人上——这才是 ${base}，不是 ${nb}。** 这句加粗。🔴绝不许这一步还在讲 ${nb} 的好/把 TA 写成 ${nb}，${nb} 只是反例、衬出 ${aux}，最终落点永远是 ${base}。
③ 【第三证人·劣势反向作证】（"焊死杠杆"四个字绝不许出现在正文）：再补一刀——TA 的劣势功能 ${infTxt} 和主导 ${dom} 在结构里 180° 死对头，一头压越低另一头翘越高——**所以 TA"${inf} 那方面"越弱，恰恰反证了全部的劲都压在 ${dom} 上。** 这句加粗。
④ 三条路收口（"最强算一次/最弱反推一次"这种公式话别甩给用户·揉进上面盘料里讲）：主导证明天赋、辅助钉死是谁、劣势反向夯实——三个不同方向，全指向同一个 ${base}。让 TA 感到"被三套逻辑同时焊死"那一震。
⑤ 这段【收尾重锤·加粗·单独成段】照抄："**你以为的缺点，从来不是缺点。天赋，是从缺点里生出来的。**" 这是本段的钩子，要重、要亮，别弱化在末尾。

## ⚔️ 三、你的重生人格（标题写"## ⚔️ 三、你的重生人格"·## 打头·只讲两副人格·别扯 buff）
① 砸结论：为活成自己，你锻出了两副人格——**${types}**，并列、不分主次。这句结论加粗。
...
② 两处铁证、各一句（"凭什么是TA"五个字绝不许出现在正文，直接讲）：一处=这两副人格的看家功能（${AfnTxt}、${BfnTxt}）本就埋在 ${base} 的功能栈里、平时被盖住；一处=盘里某类元素/星座能量重，正是它俩底料。点明"不是凭空给的，是被盘里某个痛点×渴望逼出来的"。
③ 【本段竖线重点句·绝不能少·上次这段挤成一坨没重点句=失败】本段那句最狠的话——"这两副人格不是凭空给的，是你被盘里那个痛点×渴望逼着、亲手锻出来的"——必须单独用 > 引用另起一行做成竖线重点句，让它跳出来压住场（就像 buff 段那句）。每段都要有一句这样跳出来的竖线，第三段也不许例外。
...
照抄（命门·结构一字不改·🔴这句用 > 引用单独成行做成竖线重点句，让它跳出来）：
> 它们用的是两套完全不搭界的语言，从两个完全相反的入口进去——**我们却看到了一模一样的答案：${buffLines}。两条路，同一个终点。**
照抄："不是我说你该补这个。是你的渴望在喊它，你的功能结构也在喊它，两个声音从你身体里完全不同的地方发出来，喊的却是同一个名字。**这不是好听，这是被两套独立的逻辑同时焊死的结论。**"
...
② 接一段（贴这张盘现写）：无论 TA 重生出多锋利的 ${A}、多能破局的 ${B}，无论把 ${buffLines} 点得多亮、事情做得多漂亮——TA 永远是那个 ${base}。紧跟一两句把 ${base} 那颗心点出来（用有温度的意象、别报坐标；INFJ 这类就是"看穿人心却慈悲忍着不说破、藏得最深却看得最透"，别的型按它自己的底色写）。
③ 照抄（加粗那两句）："工具放大的，从来不是工具本身，是握着它的那颗心。同样一把刀，有人拿它算计，有人拿它渡人——分别只在持刀的人。所以你不必怕自己变得太锋利，**只要那颗会心疼的心还在，你磨出多狠的本事，都还是在做一件温柔的事。**"
④ 【🔴条件填空·不许套】这一段讲"觉醒"：只有【盘里真有不配得/讨许可痛点坐标（凯龙落处、土星压宫、相关刑冲）】时，才写"TA 总在等自己够好、等世界点头才敢站上去——可那把尺是别人定的"，收到照抄句："**你真正的觉醒，不是变得更配得上，是有一天你忽然明白——你的光，不需要任何人盖章。**" 🔴盘里【没有】这类痛点坐标，绝不许套"不配得/讨许可"，换成从这张盘真实内核长出的另一记觉醒（同样升到"挣脱某个外加的枷锁/认回自己"的高度），照样收一句加粗重锤。
⑤ 收尾三句文言【单独成三行·全篇最后一锤·说完留白即止】——第一句你现写、后两句照抄一字不改：
　· 第一句【你为这张盘现写·绝不照抄范例·破雷同】：照耳根×猫腻那个气口，为 TA 写一句"挣脱 TA 自己那个枷锁/认回自己"的文言收束（因这张盘的渴望与痛点而不同，每张盘都不一样）。范例只示范味道、绝不许抄："你不必向天地借证，不必向俗世求名。"——你要换成贴这张盘的另一句，文言筋骨、加粗。
　· 后两句【每张盘都照抄·一字不改·加粗】：
