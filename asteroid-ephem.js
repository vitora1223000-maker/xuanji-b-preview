/*
 * asteroid-ephem.js ‚Äî ÂõõÂ∞èË°åÊòü(Ë∞∑Á•û/Êô∫Á•û/Â©öÁ•û/ÁÅ∂Á•û)+ÂáØÈæô ÈªÑÁªèËÆ°ÁÆó
 * Ê∫ê: astsakai/js_astro (MIT). Á≤æÂ∫¶ÂØπÊµãÊµãÊ†°È™åÈÄöËøá(‰∏âÁõò15‰Ωì,ËêΩÂ∫ß100%,Â∫¶Â∑Æ‚â§17ËßíÂàÜ,Âê´Êú´Â∫¶‰∏¥Áïå‰∏éÈÄÜË°å)„ÄÇ
 * Áî®Ê≥ï: window.asteroidEphem(utcDate) ‚Üí {ceres:{L,rx},pallas,juno,vesta,chiron}
 */
/*
 * ìVï∂åvéZä÷åWÉXÉNÉäÉvÉg version 0.22j
 * Copyright (c) 1999-2001, 2004, 2005, 2017, 2021, 2024, 2025 Yoshihiro Sakai & Sakai Institute of Astrology
 * This software is released under the MIT License.
 * http://opensource.org/licenses/mit-license.php
 * 2017/06/05[0.16j] ãOìπóvëfÇUÉpÉâî≈Ç…ëŒâû
 * 2017/09/15[0.17j] É¢ÇsÇÃåvéZéÆÇå©íºÇ∑Ç¬Ç¢Ç≈Ç…èoìTÇèëÇ≠
 * 2021/02/27[0.18j] ìÒëÃñ‚ëËÇ‹ÇÌÇËÉçÉWÉbÉNå©íºÇµ
 * 2024/12/20[0.19j] É¢ÇsåvéZéÆå©íºÇµ
 * 2025/04/14[0.20j] ãœéûç∑åvéZéÆå©íºÇµ
 * 2025/04/16[0.21j] ínï˚çPêØéûÅEâ©ìπåXéŒäpåvéZéÆå©íºÇµ
 * 2025/08/18[0.22j] èÕìÆÅEâ©ìπåXéŒäpåvéZÉçÉWÉbÉNå©íºÇµ
 */
...
// ÇªÇÃì˙ÇÃÉÜÉäÉEÉXì˙ÇåvéZÇ∑ÇÈ
function calJD(ye, mo, da, ho, mi){ // é¿êîëÃè„
	var y0 = (mo > 2) ? ye : (ye -  1);
...
function cnvJDr( JD ){ // é¿êîëÃÅ®êÆêîä¬
...
function calJDz(year, month, day){ // êÆêîä¬è„
	var yt = year;
	var mt = month;
	var dt = day;
...
// ãOìπóvëfÇUÉpÉâî≈Å®í èÌÇÃãOìπóvëf
...
// Ç‹Ç∆ÇﬂÇƒãOìπåvéZÅBï‘ílÇÕÅiâ©åoÅAâ©à‹ÅAìÆåaÅjÅB
...
// Keplerï˚íˆéÆ(M = E - e sinE)ÇâÇ≠ÅB
...
// ì˙êSà íuÇ©ÇÁínêSà íuÇ÷ÉRÉìÉoÅ[ÉgÇµÅAínêSâ©åoÇï‘Ç∑ÅB
...
// â©ìπç¿ïWånÇ©ÇÁê‘ìπç¿ïWånÇ÷ïœä∑Ç∑ÇÈÅB
...
// ãOìπóvëfÅF https://ssd.jpl.nasa.gov/planets/approx_pos.html
function calEqT( JD ) {
	const T = (JD - 2451545.0) / 36525.0;
...
// formula D : Addendum 2020 to ÅeMeasurement of the EarthÅfs rotation: 720 BC to AD 2015
...
/*
 * íËêîíËã`Å^êîäwä÷êîÉXÉNÉäÉvÉg version 0.001j at 2017/04/27
...
/*
 * ìVëÃà íuåvéZÉGÉìÉWÉìÅuÇÕÇ¢ÇœÅ[Ç÷Ç´ÇøÇ·ÇÒÅv version 1.00-j08
 ...
 * 2000/03/19[h02] àÍâûÅAëÊÇPéüåıçsç∑Ç∆èÕìÆÇÃï‚ê≥Çì¸ÇÍÇ∆Ç¢ÇΩÅB
 * 2000/07/09[h03] ê‘ìπç¿ïWånÇ÷ÇÃïœä∑Çóeà’Ç…Ç∑ÇÈÇΩÇﬂÉtÉãÉZÉbÉgÇ≈ï‘Ç∑ÇÊÇ§Ç…ÇµÇΩÅB
 * 2000/09/25[h04] ÉmÅ[ÉhÇ∆ÉäÉäÉXÇÃåvéZñ@ÇÇ∆Ç†ÇÈò_ï∂Ç…äÓÇ√Ç´ïœçXÅB
 * 2001/03/18[h05] å¥ë•Ç∆ÇµÇƒñªâ§êØÇÕAstronomical Algorithmsï˚éÆÇ≈åvéZÇ∑ÇÈÅB
 * 2001/04/23[h06] ìVëÃà íuåvéZÇÃédä|ÇØÇÇøÇÂÇ±Ç¡Ç∆ïœçX
 * 2001/07/09[h07] åıçsç∑ÇÃåvéZï˚éÆÇïœçX
 * 2001/07/13[h08] åéÇÃåvéZÇÉpÉèÅ[ÉAÉbÉvÅI
 * 2002/09/14[h09] ëæózÇ∆åéÇÃë¨ìxÇÃåvéZéÆÇí«â¡
 * 2003/01/01[h10] Ç»Ç∫Ç©ì¸Ç¡ÇƒÇ¢ÇΩÇ†ÇŸÇ»ÉoÉOÇèúãéÅB
 * 2003/01/16[h11] Ç»Ç∫Ç©ì¸Ç¡ÇƒÇ¢ÇΩÇ†ÇŸÇ»ÉoÉOÇÇ≥ÇÁÇ…èúãéÅB
 * 2003/08/29[h12] â©ìπåXéŒäpÇ™ä‘à·Ç¡ÇƒÇ‹ÇµÇΩÅB
 * 2004/01/09[h13] ÉãÉiÅ[ÉäÉ^Å[ÉìÇåvéZÇ∑ÇÈä÷êîÇí«â¡ÅB
 * 2004/01/11[h14] É\Å[ÉâÅ[ÉäÉ^Å[Éìä÷êîÇí«â¡ÅAâ©ìπåXéŒäpÇ∆èÕìÆÇï™ó£ÅB
 * 2004/01/11[h15] 1700Å`2100îNà»äOÇÃñªâ§êØÇãOìπóvëfÇ≈åvéZÇ∑ÇÈÇÊÇ§Ç…ÇµÇΩÅB
 * 2017/04/27[j01] JavaScriptÇ…à⁄êAÅB
 * 2017/05/25[j02] ñªâ§êØÇÃãOìπóvëfÇï ÉçÉWÉbÉNÇ…ì¸ÇÍë÷Ç¶ÅB
 * 2017/06/05[j03] ãOìπóvëfÇUÉpÉâî≈Ç…ëŒâû
 * 2017/08/07[j04] ñªâ§êØÇÃåvéZéÆÇ™Ç®Ç©ÇµÇ»Ç±Ç∆Ç…Ç»Ç¡ÇƒÇ¢ÇΩÅBÇ¬Ç¢Ç≈Ç…j02éÊÇËè¡Çµ
 * 2021/02/27[j05] ñªâ§êØÇÃãOìπóvëfåvéZéÆÇå©íºÇµ
 * 2024/12/20[j06] élëÂè¨òfêØÅïÉLÉçÅ[ÉìÇÃê⁄êGãOìπóvëfãﬂéóéÆëŒâû
 * 2025/02/15[j07] è¨òfêØÇÃåıçsç∑ï‚ê≥ÇëÂòfêØÇ…çáÇÌÇπÇÈ
 * 2025/07/28[j08] ê⁄êGãOìπóvëf Chebyshev ãﬂéóëΩçÄéÆÇ JPL Horizons System ê∂ê¨åãâ Ç©ÇÁçƒåvéZ
 */

// ñªâ§êØÇÃ1700Å`2100îNà»äOÇÃä˙ä‘ÇÃãOìπóvëfëIëÉtÉâÉO
var jplMode = 1;

// ìVëÃà íuÇîzóÒÇ≈ï‘Ç∑ìùäáä÷êî
...
// äeìVëÃÇÃâ©åoåvéZÅ^JDÇÕínãÖóÕäwéûäÓèÄ
...
// åıçsç∑íËêî
	...
	epos = calPositSO( T2 );
	if( pid == 1 ){ // åvéZñ⁄ïWÇ™ëæózÇÃèÍçá
		gpos[ 0 ] = epos[ 0 ] - 0.005693 / epos[ 2 ];
	} else if( pid == 2 ){ // åvéZñ⁄ïWÇ™åéÇÃèÍçá
		gpos = calPositMO(T);
	} else if ( pid <= 10 ) { // åvéZñ⁄ïWÇ™ëÂòfêØÇÃèÍçá
		switch( pid ){
			case 3:
				ppos = calPositME( T2 );
				break;
			...
			case 10:
				...
				ppos[ 0 ] += 5029.0966 / 3600.0 * T;  // ê¸å^ãﬂéóÇÃçŒç∑ï‚ê≥Ç∑ÇÈ
				break;
		}
		...
		gpos[ 0 ] += dl;
	} else { // åvéZñ⁄ïWÇ™è¨òfêØÇ©ÉLÉçÅ[ÉìÇÃèÍçá
		const mid = pid - 11;
...
// è¨òfêØà íuåvéZÅi mid: 0 - 4 Åj
function calAsteroidPosition( JD, mid ){
	// ê⁄êGãOìπóvëf Chebyshev ãﬂéóëΩçÄéÆ
	// ÉZÉåÉXÅEÉpÉâÉXÅEÉWÉÖÉmÅ[ÅEÉxÉXÉ^ÅEÉLÉçÅ[ÉìÇÃèá
	const table_a = [
	...
	// ãOìπóvëfÇ©ÇÁì˙êSà íuÇéZèo
	const x = ( JD - 2460675.0 ) / 36525.0 ;
	if ( Math.abs(x) > 1.0 ) {
		return null; // åvéZëŒè€îÕàÕäO
	}
...
/* ---- ìññ ÇÕÉ\Å[ÉâÅ[ÉäÉ^Å[ÉìÅAÉãÉiÅ[ÉäÉ^Å[ÉìÇÕégÇÌÇ»Ç¢ÅBégÇ§éûÇ…ïúäàÇ≥ÇπÇÈ
///// Return åvéZä÷êî
// Solar Return
...
// ===== ÁéÑÁéëÂπ≤ÂáÄÊé•Âè£(ÊåÇÂà∞window) =====
;(function(root){
...
root.asteroidEphem=function(utcDate){ if(!(utcDate instanceof Date)||isNaN(utcDate.getTime())) throw new Error("[asteroid-ephem] ÈúÄË¶ÅÊúâÊïàUTC Date"); return _asteroidCalc(utcDate); };
